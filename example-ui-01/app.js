(function () {
  ...
  const setupViews = new Set(["onboarding", "data-sources"]);
  let toastTimer;
  ...
  function showView(name) {
    ...
    navItems.forEach((n) => {
      const match = n.dataset.view === name;
    ...
    if (sidebar) {
      sidebar.style.display = (name === "login" || name === "landing") ? "none" : "";
    }
  ...
  document.querySelectorAll("[data-next]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (account === "workspace" || account === "personal" || account === "microsoft") {
        localStorage.setItem("is_logged_in", "true");
        localStorage.setItem("account_type", account);
        const mockEmail = account === "microsoft" ? "admin@microsoft.com" : "admin@company.com";
  ...
  document.querySelectorAll(".brand, #view-login .brand-mark").forEach((logo) => {
  ...
  function initDataSources() {
    ...
    window.handleAuth = function (providerId, btnElement, isFromModal) {
      ...
      setTimeout(() => {
        ...
        // Force the app router to switch to the data-sources view
        ...
        renderAccountsList();
        showToast(`Successfully linked ${provider.name}! Redirecting to Overview in 10 seconds...`);

        // Automatically proceed to Overview after 10 seconds
        setTimeout(() => {
          showView("dashboard");
        }, 10000);
      }, 1200);
                            ...
                            <tr>
                                <td style="font-weight: 500;">
                                    <div style="display: flex; align-items: center; gap: 8px;">
                                        ...
                                        <a href="#" class="service-link" onclick="event.stopPropagation(); event.preventDefault(); openServiceListing('${acc.email}', '${srv.name}');">${srv.name}</a>
                                    </div>
                                </td>
    ...
    window.toggleDetails = function (index) {
      // Placeholder for row expansion mechanics if desired
    ...
    function renderListingTable() {
      ...
      if (currentListingType === 'email') {
        theadTr.innerHTML = '<th>Subject</th><th>From</th><th>Date</th>';
      ...
      tbody.innerHTML = items.length > 0 ? items.map(item => `
        <tr onclick="openItemPreview('${item.title}', '${currentListingType}', '${item.content}', '${item.extra}')">
          <td><strong>${item.title}</strong></td>
          <td>${item.extra}</td>
          <td>${item.date}</td>
        </tr>
      `).join('') : `<tr><td colspan="3" style="text-align: center; color: var(--text-muted); padding: 3rem 1rem;">No items found in ${currentListingCategory}.</td></tr>`;
    }
  ...
  if (isLoggedIn) {
    document.querySelectorAll(".setup-nav-item").forEach(el => el.classList.add("hidden"));
